...
class NewContactViewModel( db: UserDao?) : BaseViewModel(db) {
    ...
    fun save(context: Context,activtity: Activity){
        uiScope.launch {
            try {
                showWaiting(context)
                val result = ContactServiceApi.retrofitService.createContactAsync(userEntity.value!!.buyerId,_contact.value!!).await()
                if(!result.isSucess){
                    showMadal(context,result.errorText)
                }
                else{
